...
public class SplitValueReader extends BaseReader<Path, Order> {
  ...
  private Order rowToOrder(String row) {
    ...
    double volume = diffAndSave(oldTotalVolume, Double.valueOf(split[9]));
    double amount = diffAndSave(oldTotalAmount, Double.valueOf(split[10]));
    double averagePrice = volume == 0 ? currentPrice : amount / volume / (name.toUpperCase().startsWith("IF") ? 300 : 10);
  ...
  private double diffAndSave(Wrapper<Double> wrap, double newValue) {
